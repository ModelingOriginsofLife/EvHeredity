...
void iterateHenon(Site *Grid, int idx)
{   
	if (idx<NSITES*NVAR)
	{   
		int site = idx/(NVAR);
		...
		{
			float nx = 1 - ALPHA*pow(Grid[site].henon[var].x,2)+Grid[site].henon[var].y;
			float ny = BETA*Grid[site].henon[var].x;
			...
			Grid[site].henon[var].x = nx;//0.2*(rand()%200001-100000.0)/100000.0;
			Grid[site].henon[var].y = ny;//0.2*(rand()%200001-100000.0)/100000.0;
		}
		
		float d1 = pow(Grid[site].henon[var].x - Grid[site].fixed[0],2) + pow(Grid[site].henon[var].y - Grid[site].fixed[1],2);
		float d2 = pow(Grid[site].henon[var].x - Grid[site].fixed[2],2) + pow(Grid[site].henon[var].y - Grid[site].fixed[3],2);
...
void Contract(Site *Grid, int idx)
{
	...
	{   
		...
		Grid[site].henon[var].x += CRATE*(Grid[site].targ[var].x - Grid[site].henon[var].x); 
		//~ if (Grid[site].henon[var].x<-2) Grid[site].henon[var].x = -2;
		//~ if (Grid[site].henon[var].x>2) Grid[site].henon[var].x = 2;

		Grid[site].henon[var].y += CRATE*(Grid[site].targ[var].y - Grid[site].henon[var].y); 
		//~ if (Grid[site].henon[var].y<-2) Grid[site].henon[var].y = -2;
		//~ if (Grid[site].henon[var].y>2) Grid[site].henon[var].y = 2;
		
		int oob = 0;
		
		if (pow(Grid[site].henon[var].x,2)+pow(Grid[site].henon[var].y,2) >= 2*2)
		{
			Grid[site].henon[var].x = (rand()%2000001-1000000.0)/1000000.0;
			Grid[site].henon[var].y = (rand()%2000001-1000000.0)/1000000.0;
		}
	}
}
...
/*	int block_size = BSIZE;
	...
	iterateHenon <<< n_blocks1, block_size >>> (Grid);
...
void Init()
{
	...
	for (int i=0;i<NSITES;i++)
	{
		...
		{
			Grid[i].henon[j].x = 2*( (rand()%1000000)/1000000.0 - 0.5);
			Grid[i].henon[j].y = 2*( (rand()%1000000)/1000000.0 - 0.5);
		}
...
void Render()
{
	...
	for (int x=0;x<NSITES;x++)
	{
		...
		//float w = (Grid[x].henon[0].x+1.0)/2.0;
