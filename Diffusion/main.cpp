...
void iterateState(Site *Grid, int idx)
{   
	if (idx<NSITES*NVAR)
	{   
		int site = idx/(NVAR);
		...
		{
            float nx = 0.1*(rand()%200001-100000.0)/100000.0;
            float ny = 0.1*(rand()%200001-100000.0)/100000.0;
// henon:
//			float nx = 1 - ALPHA*pow(Grid[site].state[var].x,2)+Grid[site].state[var].y;
//			float ny = BETA*Grid[site].state[var].x;
			...
			Grid[site].state[var].x += nx;
			Grid[site].state[var].y += ny;
		}
		
		float d1 = pow(Grid[site].state[var].x - Grid[site].fixed[0],2) + pow(Grid[site].state[var].y - Grid[site].fixed[1],2);
		float d2 = pow(Grid[site].state[var].x - Grid[site].fixed[2],2) + pow(Grid[site].state[var].y - Grid[site].fixed[3],2);
...
void Contract(Site *Grid, int idx)
{
	...
	{   
		...
		Grid[site].state[var].x += CRATE*(Grid[site].targ[var].x - Grid[site].state[var].x); 
		//~ if (Grid[site].state[var].x<-2) Grid[site].state[var].x = -2;
		//~ if (Grid[site].state[var].x>2) Grid[site].state[var].x = 2;

		Grid[site].state[var].y += CRATE*(Grid[site].targ[var].y - Grid[site].state[var].y); 
		//~ if (Grid[site].state[var].y<-2) Grid[site].state[var].y = -2;
		//~ if (Grid[site].state[var].y>2) Grid[site].state[var].y = 2;
		
		int oob = 0;
		
		if (pow(Grid[site].state[var].x,2)+pow(Grid[site].state[var].y,2) >= 2*2)
		{
			Grid[site].state[var].x = (rand()%2000001-1000000.0)/1000000.0;
			Grid[site].state[var].y = (rand()%2000001-1000000.0)/1000000.0;
		}
	}
}
...
/*	int block_size = BSIZE;
	...
	iterateState <<< n_blocks1, block_size >>> (Grid);
...
void Init()
{
	...
	for (int i=0;i<NSITES;i++)
	{
		...
		{
			Grid[i].state[j].x = 2*( (rand()%1000000)/1000000.0 - 0.5);
			Grid[i].state[j].y = 2*( (rand()%1000000)/1000000.0 - 0.5);
		}
...
void Render()
{
	...
	for (int x=0;x<NSITES;x++)
	{
		...
		//float w = (Grid[x].state[0].x+1.0)/2.0;
